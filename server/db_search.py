...
def db_text_search(request, db, ProductsV2, ImagesV2Skinny, ImagesV2):
    ...
    # style_search_cats = [req_tag for req_tag in string_list_clean if req_tag in style_cats]
    # length_search_cats = [req_tag for req_tag in string_list_clean if req_tag in length_cats]
    # color_pattern_search_cats = [req_tag for req_tag in string_list_clean if req_tag in color_pattern_cats]
    # filter_search_cats = [req_tag for req_tag in string_list_clean if req_tag in filter_cats]
    ...
    #             and_(*conditions_filter_cats),
    #         )
    #     ).order_by(func.random()).limit(result_limit - len(img_table_query_results)).all())
    #
    #     print(f'{len(query_results_relaxed)} RELAXED RESULTS ADDED')
    #
    #     img_table_query_results += query_results_relaxed
